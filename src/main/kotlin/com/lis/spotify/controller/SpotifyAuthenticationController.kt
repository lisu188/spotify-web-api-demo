...
class SpotifyAuthenticationController(
    ...
    companion object {
        private val logger: Logger = LoggerFactory.getLogger(SpotifyAuthenticationController::class.java)
    ...
    fun getCurrentUserId(token: AuthToken): String? {
        ...
        return try {
            val response = restTemplateBuilder.build().exchange<User>(
    ...
    fun callback(code: String, response: HttpServletResponse): String {
        ...
        val tokenUrl = UriComponentsBuilder.fromHttpUrl(TOKEN_URL).queryParam("grant_type", "authorization_code")
            .queryParam("code", code).queryParam("redirect_uri", CALLBACK).build().toUri()
    ...
    fun authorize(
        ...
        val builder = UriComponentsBuilder.fromHttpUrl(AUTH_URL).queryParam("response_type", "code")
            .queryParam("client_id", CLIENT_ID).queryParam("scope", SCOPES).queryParam("redirect_uri", CALLBACK)
